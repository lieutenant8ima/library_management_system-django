...
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def delbook_view(request,pk):
    
    form=forms.BookForm()
    if request.method=='POST':
        #now this form have data from html
        form=forms.BookForm.get(pk=pk)
        if form.is_valid():
            form.delete()
            return render(request,'library/bookdeleted.html')
    return render(request,'library/delbook.html',{'form':form})
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            send_mail(str(name)+' || '+str(email),message, EMAIL_HOST_USER, ['satishjadhavds@gmail.com'], fail_silently = False)
